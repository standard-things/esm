...
function acornParse(code) {
  var parser = new acorn.Parser(acornOptions, code);
  parser.strict = false;
  return parser.parse();
}
...
var importExportVisitor = new types.PathVisitor.fromMethodsObject({
  ...
  getBlockBodyPath: function (path) {
    ...
    while (stmtPath) {
      ...
      if (n.Statement.check(stmtPath.value)) {
        ...
        this.magicString
          .insertLeft(insertCharIndex = bodyPath.value.start, "{")
  ...
  visitExportDefaultDeclaration: function (path) {
    ...
    if (dd.id && (dd.type === "FunctionDeclaration" ||
      ...
      if (this.modifyAST) {
        path.replace(this._buildModuleExportDefault(declPath.value));
      }
    }
  },

  _buildModuleExportDefault: function (declaration) {
    var moduleExport = b.memberExpression(
      b.identifier("module"),
      b.identifier("export"),
      false
    );

    var exportsDefault = b.memberExpression(
    ...
    return b.expressionStatement(
      b.callExpression(moduleExport, [
        this._buildStringLiteral("default"),
        b.assignmentExpression("=", exportsDefault, declaration)
      ])
    );
  },

  _buildStringLiteral: function (string) {
    var ast = this.parse(JSON.stringify(string));
    if (ast.type === "File") ast = ast.program;
    assert.strictEqual(ast.type, "Program");
