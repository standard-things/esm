...
function compile(code, options) {
  ...
  assignmentVisitor.visit(
    ...
    importExportVisitor.exportedSymbols
  );
...
var importExportVisitor = new types.PathVisitor.fromMethodsObject({
  reset: function (rootPath, code) {
    ...
    this.exportedSymbols = Object.create(null);
  },
  ...
  visitExportNamedDeclaration: function (path) {
    ...
    if (dd) {
      ...
      if (specifierMap) {
        utils.assign(this.exportedSymbols, specifierMap);
      }
...
var assignmentVisitor = new types.PathVisitor.fromMethodsObject({
  reset: function (rootPath, magicString, exportedSymbols) {
  ...
  _assignmentHelper: function (path, childName) {
    ...
    if (child.type === "Identifier" &&
        hasOwn.call(this.exportedSymbols, child.name)) {
      this.magicString.insertRight(
        path.node.start,
        "module.export(" + JSON.stringify(child.name) + ","
      );

      this.magicString.insertLeft(path.node.end, ")");
    }
  }
});
