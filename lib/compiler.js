...
function compile(code, options) {
  var ast = options && options.ast || parse(code);
...
var importExportVisitor = new types.PathVisitor.fromMethodsObject({
  reset: function (rootPath, code, options) {
    ...
    this.generateLetDeclarations = !! (
      options && options.generateLetDeclarations);
    this.modifyAST = !! (options && options.ast);
  },
  ...
  preserveChild: function (path, childName) {
    if (childName) {
      ...
      if (this.modifyAST) {
        // Replace the given path with nothing, effectively removing it.
        path.replace();
      }
    }
  ...
  finalizeHoisting: function () {
    this.bodyPaths.forEach(function (bodyPath) {
      ...
      if (parts.length > 0) {
        ...
        if (this.modifyAST) {
          var insertArgs = parse(codeToInsert).body;
