...
function toModuleImport(source, specifierMap, uniqueKey) {
  ...
  locals.forEach(function (local, i) {
    ...
    if (imported === "*") {
      if (local.indexOf("exports.") === 0) {
        // When we're assigning all exported properties to a single
        // property of the exports object, it's because we're handling an
        // ExportNamespaceSpecifier. As with ImportNamespaceSpecifier, we
        // need to initialize the target object somewhere, but (unlike
        // ImportNamespaceSpecifier) we don't have a variable declaration
        // where we can easily initialize it. Instead, we initialize the
        // exports.* property as the argument to a .bind method call on
        // the setter function, so that we can refer to the target object
        // as `this` inside the setter function.
        bind = ".bind(" + local + "={})";
        local = "this";
      }
