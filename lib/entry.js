...
const entrySym = Symbol.for("__esmEntry");
...
function get(exported) {
  if (utils.isObjectLike(exported) &&
      hasOwn.call(exported, entrySym)) {
    return exported[entrySym];
  }
  return null;
}
...
function getOrCreate(exported, owner) {
  ...
  if (hasOwn.call(exported, entrySym)) {
    return exported[entrySym];
  }

  const entry = new Entry(exported, owner);
  exported[entrySym] = entry;
  return entry;
}
...
function onLoaded() {
  if (! this.loaded) {
    this.loaded = true;
    this.namespaces.forEach(Object.seal);
  }
}

Ep.onLoaded = onLoaded;
...
function forEachSetter(entry, names, callback) {
  ...
  for (let i = 0; i < nameCount; ++i) {
    ...
    for (let j = 0; j < keyCount; ++j) {
      const key = keys[j];
      const value = getExportByName(entry.exports, name);
