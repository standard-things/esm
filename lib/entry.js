...
class Entry {
  ...
  addSetters(parent, setters, key, namespaces) {
  ...
  runGetters() {
    let i = -1
    const names = Object.keys(this.getters)
    const nameCount = names.length
  ...
  runSetters(names) {
    ...
    let parentIDCount = parentIDs.length
    ...
    while (parentIDCount--) {
      ...
      const parent = parents[parentIDs[parentIDCount]]
  ...
  while (nameCount--) {
    ...
    while (keyCount--) {
      ...
      if (name === "*") {
        ...
        let valueNameCount = valueNames.length

        while (valueNameCount--) {
          const valueName = valueNames[valueNameCount]
