...
const esmSymbol = Symbol.for("__esmWrapper");
...
function createMap(object, key) {
  ...
  // Store the wrapper map as object[esmSymbol][key] rather than on the
...
function createStore(object) {
  return object[esmSymbol] = new FastObject;
}
...
function getRaw(func) {
  return typeof func === "function" && hasOwn.call(func, esmSymbol)
    ? func[esmSymbol]
    : func;
}
...
function getStore(object) {
  return hasOwn.call(object, esmSymbol)
    ? object[esmSymbol]
    : null;
}
