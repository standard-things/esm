...
class ImportExportVisitor extends Visitor {
  ...
  visitCallExpression(path) {
    ...
    if (callee.type === "Import") {
      overwrite(this, callee.start, callee.end, moduleAlias + ".import")
    }
  ...
  visitExportAllDeclaration(path) {
    ...
    const hoistedCode = pad(
      this,
      moduleAlias + ".watch(" + getSourceString(this, decl),
      decl.start,
      decl.source.start
    ) + pad(
      this,
      ',[["*",' + moduleAlias + ".makeNsSetter()]]);",
      decl.source.end,
      decl.end
    )
  ...
  visitExportDefaultDeclaration(path) {
    ...
    if (dd.id && (dd.type === "FunctionDeclaration" ||
      ...
      let prefix = moduleAlias + ".exportDefault("
      let suffix = ");"
      ...
      if (decl.source) {
        if (specifierMap) {
          ...
          while (++i < nameCount) {
            ...
            while (++j < localCount) {
              addToSpecifierMap(newMap, locals[j], moduleAlias + ".exports." + exported)
            }
          }
