...
class ImportExportVisitor extends Visitor {
  finalizeHoisting() {
    ...
    while (++i < infoCount) {
      ...
      if (bodyInfo.parent.type === "Program") {
        const decl = this.generateLetDeclarations ? "const " : "var "
        if (this.exportsAlias !== "exports") {
          codeToInsert += decl + this.exportsAlias + "=exports;"
        }
        if (this.moduleAlias !== "module") {
          codeToInsert += decl + this.moduleAlias + "=module;"
        }
      }
  ...
  reset(rootPath, code, options) {
    ...
    this.exportsAlias = getOption(options, "exportsAlias")
  ...
  visitExportNamedDeclaration(path) {
    ...
    if (dd) {
      ...
      if (dd.id && (type === "ClassDeclaration" ||
        ...
        let ddCount = dd.declarations.length

        while (ddCount--) {
          const names = utils.getNamesFromPattern(dd.declarations[ddCount].id)
          let nameCount = names.length

          while (nameCount--) {
            addNameToMap(specifierMap, names[nameCount])
          }
        }
      }
      ...
      if (decl.source) {
        if (specifierMap) {
          ...
          while (nameCount--) {
            ...
            while (localCount--) {
              addToSpecifierMap(newMap, locals[localCount], "exports." + exported)
            }
          }
...
function addExportedLocalNames(visitor, specifierMap) {
  ...
  while (nameCount--) {
    ...
    while (localCount--) {
      ...
      exportedLocalNames[locals[localCount]] = true
    }
  }
}
...
function computeSpecifierMap(specifiers) {
  let specifierCount = specifiers.length
  const specifierMap = Object.create(null)

  while (specifierCount--) {
    const s = specifiers[specifierCount]
