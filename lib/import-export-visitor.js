...
class ImportExportVisitor extends Visitor {
  finalizeHoisting() {
    ...
    let codeToInsert = this.bodyInfo.hoistedPrefixString
      ...
      if (decl.source) {
        if (specifierMap) {
          ...
          while (++i < nameCount) {
            ...
            while (++j < localCount) {
              addToSpecifierMap(newMap, locals[j], this.exportsAlias + "." + exported)
            }
          }
...
function toModuleImport(visitor, code, specifierMap) {
  ...
  while (++i < nameCount) {
    ...
    code +=
      // Generate shorthand functions, instead of arrow functions,
