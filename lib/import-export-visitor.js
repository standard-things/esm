...
IEVp.getBlockBodyInfo = function (path) {
  ...
  var bodyInfo = parent._bodyInfo =
    parent._bodyInfo || Object.create(null);

  if (hasOwn.call(bodyInfo, "_body")) {
    assert.strictEqual(bodyInfo._body, body);

  } else {
    bodyInfo._body = body;
    bodyInfo._insertCharIndex = insertCharIndex;
    bodyInfo._insertNodeIndex = insertNodeIndex;
    bodyInfo._hoistedExportsMap = Object.create(null);
    bodyInfo._hoistedExportsString = "";
    bodyInfo._hoistedImportsString = "";
...
IEVp.hoistExports = function (exportDeclPath, mapOrString, childName) {
  ...
  if (utils.isPlainObject(mapOrString)) {
    ...
    bodyInfo._hoistedExportsString += mapOrString;
  }
...
IEVp.hoistImports = function (importDeclPath, hoistedCode, childName) {
  ...
  bodyInfo._hoistedImportsString += hoistedCode;
  return this;
};
...
IEVp.finalizeHoisting = function () {
  this.bodyInfos.forEach(function (bodyInfo) {
    ...
    const namedExports = toModuleExport(bodyInfo._hoistedExportsMap);
    if (namedExports) {
      parts.push(namedExports);
    }

    if (bodyInfo._hoistedExportsString) {
      parts.push(bodyInfo._hoistedExportsString);
    }

    if (bodyInfo._hoistedImportsString) {
      parts.push(bodyInfo._hoistedImportsString);
    }
    ...
    if (parts.length > 0) {
      ...
      if (this.modifyAST) {
        ...
        spliceArgs.unshift(bodyInfo._insertNodeIndex, 0);
        const body = bodyInfo._body;
        body.splice.apply(body, spliceArgs);
      }
    }

    delete bodyInfo._body;
    delete bodyInfo._insertCharIndex;
    delete bodyInfo._insertNodeIndex;
    delete bodyInfo._hoistedExportsMap;
    delete bodyInfo._hoistedExportsString;
    delete bodyInfo._hoistedImportsString;
  }, this);
